...
public static class RandomUtilities
{
    ...
    private static Dictionary<Type, Func<IGrainFactory, Type, bool, object, GrainReference>>
        GrainReferenceTypeSwitch { get; } =
        new()
        {
            [typeof(Guid)] = (grainFactory, type, keyExtension, state) =>
            {
                ...
                var grainId = GetRandom<Guid>();
                var grain = string.IsNullOrEmpty(extension)
                    ? grainFactory.GetGrain(type, grainId)
                    : grainFactory.GetGrain(type, grainId, extension);
            ...
            [typeof(string)] = (grainFactory, type, keyExtension, state) =>
            {
                ...
                var grain = grainFactory.GetGrain(type, grainId);
                return (GrainReference)grain;
            }
        };
    ...
    public static T GetRandom<T>(Range<long> range = null)
    {
        ...
        if (RandomGenerators.TryGetValue(typeof(T), out randomGenerator))
        {
            ...
            if ((typeof(T) == typeof(string)))
            {
                ...
                return ((Func<object, T>)randomGenerator)(
                    Tuple.Create(range ?? new Range<long>(symbolsDefaultCount, symbolsDefaultCount), symbols));
            }
    ...
    /// <returns>Random value of the given type.</returns>
    /// <exception cref="ArgumentException"/>.
    internal static GrainReference GetRandomGrainReference<TGrainKey, TGrainGeneric>(IGrainFactory grainFactory,
        bool keyExtension)
    {
        Func<IGrainFactory, Type, bool, object, GrainReference> func;
        if (GrainReferenceTypeSwitch.TryGetValue(typeof(TGrainKey), out func))
        {
            ...
            return func(grainFactory, typeof(TGrainGeneric), keyExtension,
                Tuple.Create(new Range<long>(symbolsDefaultCount, symbolsDefaultCount), symbols));
        }
    ...
    /// <exception cref="ArgumentException"/>.
    internal static GrainReference GetRandomGrainReference<TGrainKey, TGrainGeneric>(IGrainFactory grainFactory,
        SymbolSet symbolSet, long symbolCount = 15, bool keyExtension = false)
    {
        if (symbolSet == null)
        {
            throw new ArgumentNullException(nameof(symbolSet));
        ...
        if (GrainReferenceTypeSwitch.TryGetValue(typeof(TGrainKey), out var func))
        {
            return func(grainFactory, typeof(TGrainGeneric), keyExtension,
    ...
    /// <returns>Random value of the given type.</returns>
    /// <exception cref="ArgumentException"/>.
    internal static GrainReference GetRandomGrainReference<TKey>(IGrainFactory grainFactory,
        bool extensionKey = false)
    {
        return GetRandomGrainReference<TKey, IPersistenceDummyTestGrain>(grainFactory, extensionKey);
    }
}