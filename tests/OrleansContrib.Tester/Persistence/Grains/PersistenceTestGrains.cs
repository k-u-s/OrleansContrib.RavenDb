...
internal static class TestRuntimeEnvironmentUtility
{
    public static string CaptureRuntimeEnvironment()
    {
        ...
        return
            $"   TaskScheduler={TaskScheduler.Current}{Environment.NewLine}   WorkerPoolThread={Thread.CurrentThread.Name}{Environment.NewLine}   Thread.CurrentThread.ManagedThreadId={Thread.CurrentThread.ManagedThreadId}{Environment.NewLine}   StackTrace={Environment.NewLine}   {callStack}";
    }
}
...
public class PersistenceDummyTestGrain : Grain, IPersistenceDummyTestGrain { }

[Orleans.Providers.StorageProvider(ProviderName = TestConstants.StorageProviderDefault)]
public class PersistenceTestGrain : Grain<PersistenceTestGrainState>, IPersistenceTestGrain
{
    ...
    {
        IGrainStorage grainStorage = this.GetGrainStorage(ServiceProvider);
...
[Orleans.Providers.StorageProvider(ProviderName = TestConstants.StorageProviderErrorInjector)]
...
public class PersistenceUserHandledErrorGrain : Grain<PersistenceTestGrainState>, IPersistenceUserHandledErrorGrain
{
    ...
    public async Task DoWrite(int val, bool recover)
    {
        ...
        catch (Exception exc)
        {
            ...
            logger.Warn(0, $"Grain is handling error in DoWrite - Resetting value to {original}", exc);
    ...
    public async Task<int> DoRead(bool recover)
    {
        ...
        catch (Exception exc)
        {
            ...
            logger.Warn(0, $"Grain is handling error in DoRead - Resetting value to {original}", exc);
            State = (PersistenceTestGrainState)original;
        }
        return State.Field1;
    }
...
[Orleans.Providers.StorageProvider(ProviderName = TestConstants.StorageProviderMissing)]
public class BadProviderTestGrain : Grain<PersistenceTestGrainState>, IBadProviderTestGrain
{
    ...
    public override Task OnActivateAsync()
    {
        logger.Warn(1, "OnActivateAsync");
        return Task.CompletedTask;
    }

    public Task DoSomething()
    {
        logger.Warn(1, "DoSomething");
...
public class PersistenceNoStateTestGrain : Grain, IPersistenceNoStateTestGrain
{
    ...
    public override Task OnActivateAsync()
    {
        logger.Info(1, "OnActivateAsync");
        return Task.CompletedTask;
    }

    public Task DoSomething()
    {
        logger.Info(1, "DoSomething");
        return Task.CompletedTask;
    }
}
...
[Orleans.Providers.StorageProvider(ProviderName = TestConstants.StorageProviderForTest)]
public class GrainStorageTestGrain : Grain<PersistenceTestGrainState>,
    IGrainStorageTestGrain, IGrainStorageTestGrainLongKey, IGrainStorageTestGrainStringKey
{
    public override Task OnActivateAsync()
    {
        return Task.CompletedTask;
...
public class GrainStorageTestGrainExtendedKey : Grain<PersistenceTestGrainState>,
    IGrainStorageTestGrainGuidExtendedKey, IGrainStorageTestGrainLongExtendedKey
{
    public override Task OnActivateAsync()
    {
        return Task.CompletedTask;
...
[Orleans.Providers.StorageProvider(ProviderName = TestConstants.StorageProviderMemory)]
[Reentrant]
public class ReentrentGrainWithState : Grain<StateForIReentrentGrain>, IReentrentGrainWithState
{
    ...
    private void CheckRuntimeEnvironment()
    {
        ...
        {
            var errorMsg =
                $"Found out that this grain is already in the middle of execution. Single threaded-ness violation!\n{TestRuntimeEnvironmentUtility.CaptureRuntimeEnvironment()}";
            logger.Error(1, $"\n\n\n\n{errorMsg}\n\n\n\n");
            throw new Exception(errorMsg);
            //Environment.Exit(1);
        }

        var context = RuntimeIdentity;
